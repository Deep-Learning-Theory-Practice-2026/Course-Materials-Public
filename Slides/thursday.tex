...
\begin{frame}{Lecture Overview}
  \begin{block}{Objectives for Today}
    \begin{itemize}
        ...
        \item Resolve the \textbf{Early Stopping Paradox} by making the network depth a continuously learnable parameter.
...
\begin{frame}{Generic Layer Transitions}
    ...
    \end{align*}
    where $u_l \in \mathbb{R}^d$ is the state vector at layer $l$, and $\mathcal{F}$ is the parameterised transformation block.
...
\begin{frame}{Deriving the Continuous ODE}
    ...
    \end{align*}
    This algebraic manipulation proves that a ResNet layer is exactly the \textbf{forward Euler discretisation} of an ODE!
\end{frame}

\begin{frame}{The Initial Value Problem}
    If we allow the step size $\Delta t \to 0$ and the number of layers to approach infinity (whilst keeping the total "network time" horizon $T$ fixed), the sequence of hidden states transitions into a continuous trajectory.
...
\begin{frame}{Supervised Learning as Optimal Control}
    By viewing the network as a continuous dynamical system, the training process is mathematically transformed into an \textbf{optimal control problem}. 
    ...
    \begin{theoremblock}{The Generic Cost Functional}
        ...
        \end{align*}
    \end{theoremblock}
    where $\mathcal{M}$ is the terminal cost (e.g., classification loss) and $\mathcal{R}$ is the running cost (e.g., Tikhonov weight regularisation).
\end{frame}
...
\begin{frame}{Example: Training a Classifier on Data}
    ...
    \end{align*}
    where $\mathcal{C}$ is an activation function (like the logistic sigmoid introduced on Monday), and $W$ and $\mu$ are additional terminal weights. 
...
\begin{frame}{The Hamiltonian \& Pontryagin's Minimum Principle}
    We can encapsulate the required equations in a single functional called the \textbf{Hamiltonian}, $\mathcal{H}$, yielding
    \begin{align*}
    ...
    \end{align*}
    By computing the partial derivatives of the Hamiltonian, we recover the foundational set of continuous optimality conditions:
    \begin{align*}
...
\begin{frame}{ODENets \& Learnable Time Steps}
    ...
    \vspace{0.5em}
    In the ODENet architecture, the control set is expanded to $\Theta = (W, \beta, \alpha)$, allowing the network to dynamically adjust the integration step size. 
...
\begin{frame}{Empirical Results: ODENets}
    When the simplex constraint is applied, the network systematically drops unnecessary integration steps. This effectively compresses a deep 20-layer network down to just 2 or 3 active transformational layers without sacrificing accuracy.
    
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.6\textwidth]{optimalcontrol/example_squares2d_20layers_backtracking1_seed1_timesteps}
...
\begin{frame}{Variational Network Architecture}
    This architecture fuses rigorous theoretical physics (skip connections enforcing the measurement operator $K$) with highly parameterised expressiveness (learned convolutional filters $D_i$ and non-linear RBF potentials $\rho_i$).
    
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.85\textwidth]{varnetworks/figure1-vn_overview}
...
\begin{frame}{Interpreting Learned Filters}
    ...
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.75\textwidth]{varnetworks/figure9-parameters}
...
\begin{frame}{Robust Generalisation to Unseen Pathologies}
    ...
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.85\textwidth]{varnetworks/figure7-result_kneeprotocol}
...
\begin{frame}{Closing the Loop: DEQs vs. Bilevel Learning}
    ...
    \vspace{0.5em}
    As shown by \citet{riccio2022regularization}, Bilevel Learning is actually a mathematically restricted subclass of Deep Equilibrium Models!
    ...
    \begin{itemize}
        ...
        \item \textbf{DEQs:} Learn an unconstrained, non-conservative map, offering vastly more flexible and aggressive fixed-point dynamics.
    \end{itemize}
\end{frame}
