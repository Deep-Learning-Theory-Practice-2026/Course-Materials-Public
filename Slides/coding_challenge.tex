...
\begin{frame}{Solving the Baseline: The Condat-Vu Algorithm}
...
\begin{frame}{Condat-Vu Iterations for Deblurring}
    Let $p$ be a dual variable (a vector field corresponding to the image gradients). We alternate updates with step sizes $\tau$ (primal) and $\sigma$ (dual):
    ...
    \begin{itemize}
        ...
        \item $\operatorname{proj}_{B_\alpha}$ projects the dual variable pointwise onto an $\ell_2$-ball of radius $\alpha$.
        \item \textbf{Convergence condition:} $\tau \left( \frac{\|A\|^2}{2} + \sigma \|\nabla\|^2 \right) \leq 1$.
    \end{itemize}
\end{frame}
...
\begin{frame}{Registration \& Leaderboard}
    ...
    \begin{itemize}
        ...
        \item Sign up in groups of \textbf{ideally 3} (or 2 if necessary). Note that a maximum of 7 teams is allowed in total.
...
`-- README.md
        ...
        \textbf{What you need to edit:}
        \begin{itemize}
            \item \texttt{model.py}: Define your PyTorch network architecture here.
            \item \texttt{predict.py}: Implement the inference logic (loading the image, applying the model, saving the output).
            \item \texttt{requirements.txt}: Add any extra Python libraries required for inference.
            \item \texttt{model\_weights.pth}: Upload your trained model weights.
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Constraints \& Requirements}
    \begin{alertblock}{Strict Rules for the Auto-Grader}
        \begin{itemize}
            \item \textbf{Model Size:} The \texttt{model\_weights/} directory must be $\le$ \textbf{100 MB}.
...
\begin{frame}{Testing and Submission Workflow}
    \begin{center}
    \begin{tikzpicture}[
        box/.style={draw, thick, fill=uclblue!20, minimum width=2.5cm, minimum height=1cm, align=center, rounded corners},
        arr/.style={->, thick, >=latex}
    ]
        \node[box] (train) at (0,0) {1. Train Locally};
        \node[box] (test) at (4,0) {2. Test \texttt{predict.py}};
        \node[box] (push) at (8,0) {3. Push to \texttt{main}};
        \node[box, fill=darkgreen!20, minimum height=1.5cm] (grade) at (8,-2.5) {4. GitHub Action\\Evaluates \& Scores};
    ...
    \vspace{0.5em}
    \textbf{Git Etiquette:} The grading pipeline runs automatically on pushes to \texttt{main}. Please avoid pushing every single typo. Only push when you have a significant update or a new model version to test on the leaderboard.
\end{frame}
...
\begin{frame}{Friday Presentations}
    ...
    \begin{itemize}
        \item \textbf{First Block:} 11:00 -- 13:00
        \item \textbf{Second Block:} 14:00 -- 16:00
    \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
    \begin{itemize}
        ...
        \item \textbf{The Baseline:} Total Variation regularisation solved via Condat-Vu primal-dual splitting.
        \item \textbf{The Goal:} Beat the baseline using PyTorch and \texttt{deepinv}, while respecting the 10 MB and CPU runtime constraints.
    \end{itemize}
