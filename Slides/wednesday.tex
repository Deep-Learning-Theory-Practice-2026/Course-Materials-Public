...
\begin{frame}{The End-to-End Problem (Recap)}
    On Tuesday, we derived highly powerful unrolled architectures like the Learned Primal-Dual (LPD) network. While these models represent the state-of-the-art for specific tasks, they suffer from a fundamental flaw: \textbf{Rigidity}.
    
    \vspace{0.5em}
    By unrolling the iterations, the network incorporates the forward operator $A$ directly into its computational graph during training.
    ...
    \vspace{0.5em}
    Can we decouple the neural network from the physical operator $A$, creating a truly \emph{modular} system?
\end{frame}

\begin{frame}{Variable Splitting as a DL Framework}
    To achieve true modularity, we revisit classical variable splitting algorithms. We previously discussed ADMM and PDHG, but today we introduce a simpler method known as \textbf{Half-Quadratic Splitting (HQS)}.
    
    Consider solving our standard variational problem via HQS:
    \begin{align*}
        \min_{x, v} \left\{ \frac{1}{2}\|Ax - y^\delta\|_2^2 + \alpha \mathcal{J}(v) + \frac{\mu}{2} \|x - v\|_2^2 \right\} \, .
    \end{align*}
    Here, we introduce an auxiliary variable $v$ and penalise its distance to the image $x$ with a parameter $\mu > 0$. As $\mu \to \infty$, $x$ converges to $v$, solving the original problem.
\end{frame}
...
\begin{frame}{The Plug-and-Play (PnP) Heuristic}
    The core insight of the \textbf{Plug-and-Play} heuristic is that we do not need to manually define a mathematical prior $\mathcal{J}$ and derive its proximal operator.
    
    \vspace{0.5em}
    \begin{theoremblock}{The PnP Strategy}
        We simply replace the proximal operator with a general, state-of-the-art neural network denoiser $D_\sigma(\cdot)$ that has been pre-trained on vast datasets of natural images:
        \begin{align*}
    ...
    \vspace{0.5em}
    By ``plugging in'' highly advanced models like BM3D, DnCNN, UNet, or DRUNet, we leverage deep learning purely as a universal spatial prior, maintaining total independence from the specific measurement physics $A$.
\end{frame}
...
\begin{frame}{From HQS to ADMM}
    While HQS is incredibly simple, the Alternating Direction Method of Multipliers (ADMM) handles constraints properly and avoids the pitfalls of penalty methods (where $\mu \to \infty$ is needed to strictly enforce $x=v$).
    
    \vspace{0.5em}
    Recall the Augmented Lagrangian for ADMM, where we substitute the original Lagrange multiplier directly with a scaled dual variable $w$ yielding
    \begin{align*}
...
\begin{frame}{PnP-ADMM Algorithm}
    ...
    \For{$k = 0, 1, \dots$}
        ...
        \State \textbf{2. Neural Denoising:} $v^{k+1} = D_{\sigma}(x^{k+1} + w^k)$
...
\begin{frame}{Practical Parameter Tuning}
    ...
    \vspace{0.5em}
    In theory, for the MAP interpretation to hold, we require $\sigma_k = \sqrt{\alpha/\mu}$.
...
\begin{frame}{Iterative Regularisation \& Constrained Formulation}
    Instead of relying on heuristic noise decay schedules to restore fine textures, we can use a more mathematically principled alternative.
    
    \vspace{0.5em}
    Instead of solving the unconstrained variational problem, we target the strictly constrained formulation yielding
    \begin{align*}
        \min_{x, v} \mathcal{J}(v) \quad \text{subject to} \quad Ax = y^\delta \quad \text{and} \quad x = v \, .
    \end{align*}
    By applying the method of multipliers, we introduce two distinct scaled dual variables: $w_1$ to enforce data consistency and $w_2$ to enforce variable splitting.
\end{frame}

\begin{frame}{PnP-Bregman: Algorithm}
    This yields a powerful variant known as \textbf{PnP-Bregman}:
    
    \begin{algorithm}[H]
    \caption{Plug-and-Play Constrained ADMM (PnP-Bregman)}
    \begin{algorithmic}[1]
    \State \textbf{Initialise:} $x^0 = A^\top y^\delta$, $v^0 = y^\delta$, $w_1^0 = 0$, $w_2^0 = 0$
    \For{$k = 0, \dots, N-1$}
        \State \textbf{1. Inversion:} $x^{k+1} = (A^\top A + I)^{-1} \left( A^\top(y^\delta - w_1^k) + (v^k - w_2^k) \right)$
        \State \textbf{2. Denoising:} $v^{k+1} = D_{\sigma}(x^{k+1} + w_2^k)$
        \State \textbf{3. Dual Updates:}
        \State \quad $w_1^{k+1} = w_1^k + (A x^{k+1} - y^\delta)$ \hfill $\leftarrow$ \textit{Bregman correction}
        \State \quad $w_2^{k+1} = w_2^k + (x^{k+1} - v^{k+1})$
        \State \textbf{4. Early Stopping Check:} break if $\|Ax^{k+1} - y^\delta\|_2^2 \le \tau \sigma_{\text{noise}}^2$
    \EndFor
    \end{algorithmic}
    \end{algorithm}
\end{frame}

\begin{frame}{Advantages of the Bregman Formulation}
    The critical difference in this constrained formulation lies in the dual variable $w_1$.
    
    \vspace{0.5em}
    \begin{itemize}
        \item \textbf{Fixed Noise Level:} The dual variable $w_1$ acts as a Bregman iteration, continuously integrating the data residual. This systematically re-injects lost contrast back into the inversion step, allowing us to operate the denoiser at a \textbf{fixed} noise level $\sigma$.
        \item \textbf{Iterative Regularisation:} Because this algorithm eventually forces $Ax \to y^\delta$, it represents an iterative regularisation method. 
        \item \textbf{Early Stopping:} If run to infinity on noisy data, it will eventually fit the noise. We must halt the iteration early using Morozov's discrepancy principle \citep{morozov1966solution}.
    \end{itemize}
\end{frame}
...
\begin{frame}{1. The Optimisation Perspective}
    ...
    \vspace{0.5em}
    By Moreau's Theorem, a function $D_\sigma$ is the proximal operator of some closed, proper, convex function if and only if:
    \begin{enumerate}
...
%================================================================%
\section{Defining Losses \& The Jacobian Critique}
%================================================================%
...
\begin{frame}{Reality Check: Do CNN Denoisers have Symmetric Jacobians?}
    ...
    \begin{itemize}
        \item \textbf{Classical Denoisers:} Algorithms like Non-Local Means and BM3D rely on complex, data-dependent block-matching mechanisms. Consequently, their Jacobians are highly asymmetric.
        \item \textbf{Deep Learning Denoisers (CNNs):} Standard feed-forward networks (e.g., DnCNN, UNet) involve interleaved convolutions and non-linear activations (like ReLU). Their Jacobians are entirely asymmetric.
    \end{itemize}
...
\begin{frame}{KL Divergence as a Bregman Distance (2/2)}
    We carefully expand the inner product integral to observe
    \begin{align*}
        D_J(p, q) &= \int p(x) \log p(x) dx - \int q(x) \log q(x) dx \\
...
\begin{frame}{A Taxonomy of Generative AI}
    To implement this, we require neural network architectures that guarantee $p_\theta(x) \ge 0$ and $\int p_\theta(x) dx = 1$. We broadly divide these into Explicit and Implicit density models.
...
\begin{frame}{Marginal Likelihood as Smoothing}
    We assume the signal is corrupted by additive white Gau\ss ian noise, so the likelihood is purely Gau\ss ian. We consider the marginal distribution of the noisy data, $p_\sigma(y)$, obtained by marginalising out $x$ yielding
    \begin{align*}
        p_\sigma(y) = \int p(y | x) p(x) \, dx \, .
    \end{align*}
    
    \pause
    Notice that this marginal likelihood is mathematically equivalent to the \textbf{convolution} of the clean prior $p(x)$ with the Gau\ss ian noise distribution. Therefore, the marginal likelihood $p_\sigma(y)$ is synonymous with a continuously smoothed prior!
...
\begin{frame}{Preview: Thursday}
    ...
    \begin{itemize}
        \item \textbf{Continuous Depth:} Breaking the paradigm of explicit layer-by-layer computation by viewing deep networks as Ordinary Differential Equations (Neural ODEs).
        \item \textbf{Implicit Deep Learning:} How do we build networks with truly infinite depth and $\mathcal{O}(1)$ memory footprints? A deep dive into Deep Equilibrium Models (DEQs).
    \end{itemize}
\end{frame}
