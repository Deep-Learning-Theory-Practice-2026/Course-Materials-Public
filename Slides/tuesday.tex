...
\begin{frame}{Null-Space Networks}
    A \textbf{Null-Space Network} \citep{schwab2019deep, schwab2020big, boink2022data} isolates the data-driven model entirely within the null space of the operator $A$.
    ...
    \begin{theoremblock}{Null-Space Architecture}
        ...
        \begin{align*}
            f_\theta(y) = x_0 + (I - A^\dagger A) \mathcal{H}_\theta(x_0)
        \end{align*}
...
\begin{frame}{Proof of Strict Data Consistency}
    The network is mathematically guaranteed to be data consistent (if the data is in the range of $A$). We apply the forward operator $A$ to the network's output to observe
    
    \begin{align*}
        A f_\theta(y) &= A \left( A^\dagger y + (I - A^\dagger A) \mathcal{H}_\theta(x_0) \right) \, , \\
        &= A A^\dagger y + A(I - A^\dagger A) \mathcal{H}_\theta(x_0) \, , \\
        &= A A^\dagger y + (A - A A^\dagger A) \mathcal{H}_\theta(x_0) \, .
    \end{align*}
    By the properties of the pseudo-inverse, $A A^\dagger A = A$. Therefore, we have
    \begin{align*}
        A f_\theta(y) = A A^\dagger y + (A - A) \mathcal{H}_\theta(x_0) = A A^\dagger y \, .
    \end{align*}
    Since $A A^\dagger$ is the orthogonal projection onto the range of $A$, this yields $A f_\theta(y^\delta) = y$ if $y \in \mathcal{R}(A)$.
\end{frame}
...
\begin{frame}{Moving Beyond the Pseudo-Inverse}
    ...
    where
    \begin{itemize}
        \item $\mathcal{D}_{y^\delta}$ is the \textbf{Data Fidelity} term, ensuring the solution matches the measurements.\pause
        \item $\mathcal{J}$ is the \textbf{Regularisation} term, enforcing prior knowledge about $x^\dagger$.\pause
...
\begin{frame}{Examples of Variational Terms}
    ...
    \begin{itemize}
        \item<1-> $\frac{1}{2}\|Ax - y^\delta\|_2^2$: Standard Gau\ss ian noise assumption.
        \item<2-> $\|Ax - y^\delta\|_1$: Robust to Laplacian-distributed, heavy-tailed or outlier noise.
        \item<3-> Kullback--Leibler Divergence $\sum_{i} \left( y_i^\delta \log\left(\frac{y_i^\delta}{(Ax)_i}\right) + (Ax)_i - y_i^\delta \right)$: Used for Poisson noise (e.g., photon counting in PET).
    \end{itemize}
    
    \vspace{0.5em}
    \textbf{Regularisation $\mathcal{J}(x)$:}
    \begin{itemize}
        \item<4-> $\frac{1}{2}\|x\|_2^2$: Tikhonov regularisation \citep{tikhonov1963solution} (promotes small energy).
        \item<5-> $\|x\|_1$: Promotes sparsity (many pixels/coefficients are exactly zero).
        \item<6-> $\sup_{\| p \|_\infty \leq 1} \langle x, \operatorname{div} p \rangle \approx \|\nabla x\|_1$: Total Variation (TV) \citep{rudin1992nonlinear} (promotes piecewise constant images, preserving sharp edges).
    \end{itemize}
\end{frame}
...
\begin{frame}{The Proximal Operator}
    \only<1>{To efficiently handle non-smooth convex regularisation functions, we define the \textbf{Proximal Operator} \citep{moreau1965proximite}.}
...
\begin{frame}{The hyperparameter problem}
    In standard machine learning, we minimise an empirical risk on a training set to find model parameters. However, this process is governed by \textbf{hyperparameters} $\Theta$ (e.g., regularisation weights), which control the model's capacity and generalisation.\pause
    
    \vspace{0.5em}
    Typically, we select $\Theta$ by evaluating the trained model on a separate \textbf{validation set}. As the dimensionality of $\Theta$ grows, manual tuning or grid search becomes intractable.\pause
...
\begin{frame}{Bilevel learning for inverse problems}
    Applying this to inverse problems \citep{kunisch2013bilevel, holler2018bilevel}, suppose we have a dataset of ground-truth images $x_i^\dagger$ and noisy measurements $y_i^\delta$. We can \emph{learn} the optimal parameters $\Theta$ of our regulariser yielding
    \begin{align*}
...
\begin{frame}{Deriving the sensitivity via IFT}
    ...
    \end{align*}
    Hence, we can analytically compute gradients for hyperparameter learning by solving a single linear system at the end, regardless of how many iterations it took to find $x^\star$.
\end{frame}
...
\begin{frame}{The Unrolling Principle}
    This leads to one of the most powerful concepts in modern computational imaging: \textbf{Algorithm Unrolling} (or Unfolding) \citep{monga2021algorithm}.
