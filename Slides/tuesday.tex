...
\begin{frame}{Lecture Overview}
  \begin{block}{Objectives for Today}
    \begin{itemize}
        \item Understand the \textbf{Null Space} of forward operators and how to enforce strict data consistency using \textbf{Null-Space Networks}.
        \item Formulate inverse problems using \textbf{Variational Methods} and explore classical regularisation.
        \item Review convex and non-convex \textbf{Optimisation Algorithms}, specifically Proximal Gradient Descent (ISTA) and Primal--Dual Hybrid Gradient (PDHG).
        \item Formulate learning as a nested \textbf{Bilevel Optimisation} problem.
        \item Compute exact sensitivities for hyperparameter learning using the \textbf{Implicit Function Theorem (IFT)} and \textbf{Huber smoothing}.
        \item Learn explicit regularisation functionals via the \textbf{Network Tikhonov (NETT)} framework and restore guarantees using \textbf{Input Convex Neural Networks (ICNNs)}.
        \item Introduce the \textbf{Unrolling Principle} for truncating iterative algorithms into deep neural networks (e.g., LISTA, LPD).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Recap: The Inverse Problem}
    Recall from Monday that we are dealing with linear inverse problems of the form
    \begin{align*}
...
\begin{frame}{Orthogonal Decomposition}
    By the \textbf{Fundamental Theorem of Linear Algebra}, the domain $\mathbb{R}^n$ is the direct sum of the null space and the row space of $A$, i.e.,
    \begin{align*}
...
\begin{frame}{Examples of Variational Terms}
    ...
    \begin{itemize}
        ...
        \item $\|Ax - y^\delta\|_1$: Robust to heavy-tailed or outlier noise.
