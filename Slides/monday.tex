...
\begin{frame}{Lecture Overview}
  \begin{block}{Objectives}
    \begin{itemize}
        \item Understand foundational Neural Network Architectures (Perceptrons to MLPs).
        \item Explore Convolutional Networks and advanced structures (ResNets, UNets).
        \item Master training dynamics, loss functions, and practical optimisation.
        \item Derive Backpropagation via the Chain Rule.
        \item Analyse the pitfalls of naive end-to-end learning for inverse problems.
        \item Generalise Backpropagation to implicit layers, fixed-point problems, and continuous limits.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Shallow Neural Networks}
    To solve XOR, we add a \textbf{hidden layer}.\vspace{-0.1cm}
...
\begin{frame}{Adam: interpretation and practical caveats}
    ...
    \begin{alertblock}{Theory vs practice}
        Adam is highly effective in deep learning practice, but classical convergence guarantees can fail without additional assumptions or modifications. A common robust variant is AdamW, which decouples weight decay from the adaptive step.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Dropout in practice}
    ...
    \vspace{0.25em}
    During training, randomly zero out a fraction $(1-q)$ of activations using a Bernoulli mask:
    \begin{align*}
    ...
    \vspace{0.25em}
    \textbf{Inverted dropout} (modern standard) rescales activations:
    \begin{align*}
...
\begin{frame}{Batch normalisation in practice}
    \textbf{Placement:} Applied to pre-activations $z$ (before the nonlinearity) within a hidden layer. Typical architecture: Dense $\to$ Batch Norm $\to$ Nonlinearity $\to$ Dropout.

    \vspace{0.25em}
    For each feature/channel, compute mini-batch statistics:
    \begin{align*}
    ...
    \end{align*}
    Then normalise and apply learnable affine parameters:
    \begin{align*}
...
\begin{frame}{Batch normalisation: training vs inference}
    ...
    \vspace{0.5em}
    \textbf{Inference:} Mini-batches are unavailable (single sample or fixed batch). Instead, use \emph{running averages} accumulated during training via exponential moving average (EMA):
    \begin{align*}
...
\begin{frame}{Residual Networks (ResNets)}
    \textbf{Solution:} Introduce a "Skip Connection" \citep{he2016deep}.
    
    Instead of learning $x_{l+1} = \mathcal{F}_w(x_l)$, we learn the \textbf{residual}:
...
\begin{frame}{Encoder-Decoder Architectures and UNets}
    ...
    The skip connections $e_s \to d_s$ bypass the bottleneck, preserving high-frequency spatial detail that is irreversibly lost under downsampling.
\end{frame}
...
\begin{frame}{The Temptation of End-to-End Learning}
    ...
    \begin{block}{Moore--Penrose Pseudo-Inverse \protect\citep{ehn}}
